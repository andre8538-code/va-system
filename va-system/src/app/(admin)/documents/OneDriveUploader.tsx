"use client";
// src/app/(admin)/documents/OneDriveUploader.tsx
import { useState, useRef } from "react";
import { useRouter } from "next/navigation";

export default function OneDriveUploader({ projectId, projectName }: {
  ...
  const upload = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      const r = await fetch("/api/onedrive/upload", { method: "POST", body: form });
      ...
      <div
        ...
        {uploading ? (
          <div className="flex items-center justify-center gap-3">
            ...
            <p className="text-sm text-[#1E4D8C] font-medium">Laddar upp till OneDrive…</p>
          </div>
        ) : results.length > 0 ? (
          <div>
            {results.map(r => (
          ...
          <div>
            <p className="text-sm font-medium text-[#1A1916] mb-0.5">Dra och släpp filer för att ladda upp till OneDrive</p>
